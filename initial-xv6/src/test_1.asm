...
main(int argc, char *argv[]) {
  ...
  int x1 = getreadcount();
  23:	e8 fb 02 00 00       	call   323 <getreadcount>
  28:	89 85 74 ff ff ff    	mov    %eax,-0x8c(%ebp)
  int x2 = getreadcount();
  2e:	e8 f0 02 00 00       	call   323 <getreadcount>
  (void) read(4, buf, 1);
  ...
  int x3 = getreadcount();
  46:	e8 d8 02 00 00       	call   323 <getreadcount>
  ...
  int x4 = getreadcount();
  65:	e8 b9 02 00 00       	call   323 <getreadcount>
...
stat(const char *n, struct stat *st)
{
 ...
 251:	e8 f5 00 00 00       	call   34b <open>
  if(fd < 0)
 ...
 265:	50                   	push   %eax
 266:	e8 f8 00 00 00       	call   363 <fstat>
  close(fd);
 26b:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 26e:	89 c6                	mov    %eax,%esi
  close(fd);
 270:	e8 be 00 00 00       	call   333 <close>
  return r;
 ...
 3bc:	89 d1                	mov    %edx,%ecx
{
 ...
 42e:	56                   	push   %esi
 42f:	57                   	push   %edi
 430:	e8 f6 fe ff ff       	call   32b <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
 ...
 4a9:	e8 7d fe ff ff       	call   32b <write>
 ...
 509:	e8 1d fe ff ff       	call   32b <write>
        putc(fd, c);
      } else {
 ...
 51d:	e8 09 fe ff ff       	call   32b <write>
  for(i = 0; fmt[i]; i++){
 ...
 591:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 594:	e8 92 fd ff ff       	call   32b <write>
        ap++;
 ...
 60b:	57                   	push   %edi
 60c:	53                   	push   %ebx
 60d:	e8 19 fd ff ff       	call   32b <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 72b:	e8 63 fc ff ff       	call   393 <sbrk>
  if(p == (char*)-1)
